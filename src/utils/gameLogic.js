...
import { initialFinanceState, financeHelpers, PersonalAccountTypes, IncomeTypes } from '../types/finance.js';
...
export const gameLogic = {
  ...
  processMonthlyUpdates: (state) => {
    ...
    Object.keys(financeState.cityBudget.monthlyExpenses || {}).forEach((category) => {
      const expense = financeState.cityBudget.monthlyExpenses[category] || 0;
      const spent = financeState.cityBudget.spent?.[category] || 0;
      const allocated = financeState.cityBudget.allocated?.[category] || 0;
      financeState.cityBudget.spent[category] = Math.min(allocated, spent + expense);
    });
    ...
    (industryState.activeProjects || []).forEach((project) => {
      ...
      projectEntry.status = progress.isCompleted ? ProjectStatus.OPERATIONAL : progress.currentPhase;
